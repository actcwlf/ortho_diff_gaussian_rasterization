	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaOrthoRasterizer::OrthoRasterizer::forward(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
	...
	const bool debug) 
{
  ...
  if(P != 0)
  {  
	  CudaOrthoRasterizer::OrthoRasterizer::backward(P, degree, M, R,
	  background.contiguous().data<float>(),
	  W, H, 
	  means3D.contiguous().data<float>(),
...
{ 
  ...
  if(P != 0)
  {
	CudaOrthoRasterizer::OrthoRasterizer::markVisible(P,
		means3D.contiguous().data<float>(),
		viewmatrix.contiguous().data<float>(),
		// projmatrix.contiguous().data<float>(),
		threshold,
