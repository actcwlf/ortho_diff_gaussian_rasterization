...
__global__ void filter_preprocessCUDA(int P, int M,
	const float* orig_points,  // points的坐标值/means3D的值大致在-4~8之间，表明世界坐标系的坐标数值范围大致在此区间
	const glm::vec3* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	const float* cov3D_precomp,
	const float* viewmatrix,
	const float* projmatrix,
	const int W, int H, // pixel为单位
	const float tan_fovx, float tan_fovy,
	const float focal_x, float focal_y, // pixel为单位
	int* radii,
	float* cov3Ds,
	const dim3 grid,
	bool prefiltered)
{
	...
	float3 tp_orig = { orig_points[3 * idx], orig_points[3 * idx + 1], orig_points[3 * idx + 2] };




	float4 tp_hom = transformPoint4x4(tp_orig, projmatrix);
